...
def generate_magic_key(session_bytes: bytes):
    # pad to 16-bytes
...
def main():
    ...
    if cams is None:
        ...
        return
    choice = select("Select a camera:", choices=cams).ask()
    if choice is None:
        return
    cam_info = cams[choice]
    ...
    threading.Thread(target=listen_to_camera, args=(socket_2,), daemon=True).start()
    ...
    print(f"[TX] Streaming Loop Started (Duration: {packet_duration}s per chunk)...")
